...
export function RegisterComponent() {
    ...
    const handleSubmitRegister = async (e) => {
        ...
        axios.post(`${API_BASE_URL}/api/user`, { username,email,mobile,emailLead, "password":confirmPassword })
            .then(async response => {
                        ...
                        <Card className="max-w-full h-auto px-2">
                            <CardBody className="overflow-hidden">
                                <div className="flex flex-col gap-2 pt-3 pb-4 ">
                                    ...
                                    <p  className="text-sm text-default-500">Enter your information to create an account </p>
                                </div>
                                <form className="flex flex-col gap-3 " onSubmit={handleSubmitRegister}>
                                    ...
                                    />
                                    <Input
                                        isRequired label="Mobile number" value={mobile}
                                        onValueChange={setMobile}
                                        isInvalid={isMobileInvalid}
                                        color={mobile ? isMobileInvalid ? "danger" : "success" : "default"}
                                    ...
                                    <Input
                                        isRequired label="Email Team lead" value={emailLead}
                                        onValueChange={setEmailLead}
                                        isInvalid={isEmailLeadInvalid}
                                        color={emailLead ? isEmailLeadInvalid ? "danger" : "success" : "default"}
                                    ...
                                    {regisError && <div
                                        className="text-danger-500 text-[0.75rem] text-center"> {regisErrorMsg}</div>}
                                    ...
                                    <div className="flex gap-2 justify-end">
                                        <Button
                                            isDisabled={(email ? isEmailInvalid : true) || (mobile ? isMobileInvalid : true) || (emailLead ? isEmailLeadInvalid : true) || (confirmPassword ? isInvalidConfirmPassword : true)}
